...
def get_likelihood_fn(sde, model, inverse_scaler, hutchinson_type='Rademacher',
    ...
    def drift_fn(state, x, t):
        ...
        return rsde.sde(x, t, params=state.params_ema)[0]
    ...
    def likelihood_fn(prng, pstate, data, t_data=0., t_gauss=1.):
        ...
        solution = integrate.solve_ivp(ode_func, (jnp.clip(t_data * sde.T, a_min=eps, a_max=sde.T), jnp.clip(t_gauss * sde.T, a_min=eps, a_max=sde.T)), init, rtol=rtol, atol=atol, method=method)
        nfe = solution.nfev
        ...
        bpd += offset
        return bpd, z, (solution.t, solution.y) # nfe
