...
@jax.vmap
def kl_divergence(z):
    return jnp.absolute(z).sum(axis=-1)
...
def eval(params, batch, z, z_rng):
    def eval_model(vae):
        if batch is not None:
            images = batch  # ['image']
            recon_images, z_recon = vae(images, z_rng)
        ...
        if batch is not None:
            metrics = compute_metrics(recon_images, images, z_recon)
...
def main(argv):
    ...
    train_ds_eval = ds_builder.as_dataset(split=tfds.Split.TRAIN)
    train_ds_eval = train_ds_eval.map(prepare_image).batch(1000)

    test_ds_eval = ds_builder.as_dataset(split=tfds.Split.TEST)
    test_ds_eval = test_ds_eval.map(prepare_image).batch(1000)
    ...
    if mode == 'train':
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            if np.isnan(metrics['bce']).sum() > 0:
                break
        checkpoints.save_checkpoint(f'ckpt_cifar_ae_{FLAGS.latents}', optimizer.target, step=1, keep=np.inf)
    mode = 'output_z'
    if mode == 'output_z':
        params = checkpoints.restore_checkpoint(f'ckpt_cifar_ae_{FLAGS.latents}', params, step=1)
        for ds_str in ['eval', 'train']:
            ds = train_ds_eval if ds_str == 'train' else test_ds_eval
            zs, labels = [], []
            for batch in ds:
                rng, key = random.split(rng)
                _, z = model().apply({'params': params}, batch['image'], key)
                zs.append(z)
                labels.append(batch['label'])
            zs = np.concatenate(zs, axis=0)
            labels = np.concatenate(labels, axis=0)
            np.save(f'ae_z0_{ds_str}', {'z': zs, 'label': labels})
