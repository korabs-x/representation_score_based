...
def get_pc_conditional_sampler(sde, score_model, classifier, classifier_params, shape,
                               ...
                               continuous=False, denoise=True, eps=1e-5, config=None):
    ...
    guidance_factor = config.eval.guidance_factor
    ...
    def conditional_predictor_update_fn(rng, state, x, t, labels):
        ...
        def total_grad_fn(x, t, z0=None):
            ve_noise_scale = sde.marginal_prob(x, t)[1]
            score = score_fn(x, t)
            score['score'] = score['score'] + guidance_factor * classifier_grad_fn(x, ve_noise_scale, labels)
            return score
    ...
    def pc_conditional_sampler(rng, score_state, labels):
        ...
        _, x, x_mean = jax.lax.fori_loop(0, sde.N, loop_body, (rng, x, x))
        return inverse_scaler(x_mean if denoise else x), sde.N

    return jax.pmap(pc_conditional_sampler, axis_name='batch')


def get_conditional_sampling_fn(config, sde, model, shape, inverse_scaler, eps, probability_flow=False,
    ...
    sampling_fn = get_pc_conditional_sampler(sde,
                                             model,
                                             classifier,
                                             classifier_params,
                                             shape,
                                             predictor,
                                             corrector,
                                             inverse_scaler, config.sampling.snr,
