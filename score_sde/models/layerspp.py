...
class ResnetBlockDDPMpp(nn.Module):
    ...
    def __call__(self, x, temb=None, train=True):
        # print(("ResnetBlockDDPMpp", x.shape), file=sys.stderr)
        B, H, W, C = x.shape
        out_ch = self.out_ch if self.out_ch else C
        # print(("resnet-1", x.shape), file=sys.stderr)
        h = self.act(nn.GroupNorm(num_groups=min(x.shape[-1] // 4, 32))(x))
        # print(("resnet-2", h.shape), file=sys.stderr)
        h = conv3x3(h, out_ch)
        # print(("resnet-3", h.shape), file=sys.stderr)
        ...
        # print(("resnet-4", h.shape), file=sys.stderr)
        h = nn.Dropout(self.dropout)(h, deterministic=not train)
        # print(("resnet-5", h.shape), file=sys.stderr)
        h = conv3x3(h, out_ch, init_scale=self.init_scale)
        # print(("resnet-6", h.shape), file=sys.stderr)
        if C != out_ch:
            if self.conv_shortcut:
                x = conv3x3(x, out_ch)
                # print(("resnet-7.1", x.shape), file=sys.stderr)
            else:
                x = NIN(out_ch)(x)
                # print(("resnet-7.2", x.shape), file=sys.stderr)
...
class ResnetBlockBigGANpp(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, temb=None, train=True):
        # print(("ResnetBlockBigGANpp", x.shape), file=sys.stderr)
        B, H, W, C = x.shape
        out_ch = self.out_ch if self.out_ch else C
        h = self.act(nn.GroupNorm(num_groups=min(x.shape[-1] // 4, 32))(x))
        ...
        if self.up:
            ...
            if self.fir:
                # print(("self.fir", "yes"), file=sys.stderr)
                h = up_or_down_sampling.downsample_2d(h, self.fir_kernel, factor=2)
                x = up_or_down_sampling.downsample_2d(x, self.fir_kernel, factor=2)
            else:
                # print(("self.fir", "no"), file=sys.stderr)
                h = up_or_down_sampling.naive_downsample_2d(h, factor=2)
                x = up_or_down_sampling.naive_downsample_2d(x, factor=2)

        h = conv3x3(h, out_ch, padding=self.padding)
        # Add bias to each feature map conditioned on the time embedding
        if temb is not None:
            h += nn.Dense(out_ch, kernel_init=default_init())(self.act(temb))[:, None, None, :]
