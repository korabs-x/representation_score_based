...
class SDE(abc.ABC):
    ...
    @abc.abstractmethod
    def marginal_prob(self, x, t, params=None):
    ...
    def reverse(self, score_fn, probability_flow=False):
        ...
        class RSDE(self.__class__):
            ...
            def sde(self, x, t, params=None, z0=None):
                assert params is not None
                """Create the drift and diffusion functions for the reverse SDE/ODE."""
                drift, diffusion = sde_fn(x, t, params=params)
                score = score_fn(x, t, z0=z0)['score']
                drift = drift - batch_mul(diffusion ** 2, score * (0.5 if self.probability_flow else 1.))
                # Set the diffusion function to zero for ODEs.
                diffusion = 0. if self.probability_flow else diffusion
                return drift, diffusion

            def discretize(self, x, t, z0=None):
                """Create discretized iteration rules for the reverse diffusion sampler."""
                f, G = discretize_fn(x, t)
                rev_f = f - batch_mul(G ** 2, score_fn(x, t, z0=z0)['score'] * (0.5 if self.probability_flow else 1.))
                rev_G = 0. if self.probability_flow else G
                return rev_f, rev_G

        return RSDE()

    def lambda_mle(self, t, params=None):
        print("WARNING: lambda_mle being requested for SDE different than VP", file=sys.stderr)
        return t * 0. + 1.
...
class VPSDE(SDE):
    ...
    def sde(self, x, t, params=None):
        #assert params is not None
        b0, b1 = self.get_betas(params=params)
        beta_t = b0 + t * (b1 - b0)
    ...
    def get_betas(self, params=None):
        #assert params is not None
        # b0 = self.beta_0  # if params is None else params['sde_beta_min']
        # b1 = self.beta_1  # if params is None else params['sde_beta_max']
        b0 = self.beta_0  # if params is None else params['sde_beta']
        b1 = self.beta_1  # if params is None else self.beta_1 - params['sde_beta']
        return b0, b1
...
class subVPSDE(SDE):
    ...
    def prior_sampling(self, rng, shape, params=None):
        return jax.random.normal(rng, shape)

    def prior_logp(self, z, params=None):
        shape = z.shape
        N = np.prod(shape[1:])
        logp_fn = lambda z: -N / 2. * jnp.log(2 * np.pi) - jnp.sum(z ** 2) / 2.
        return jax.vmap(logp_fn)(z)
...
class VESDE(SDE):
    ...
    def discretize(self, x, t, params=None):
